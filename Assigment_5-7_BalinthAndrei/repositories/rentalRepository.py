...
class RentalRepo():
    # creates an empty list which will holds all the rentals
    def __init__(self):
        self._list = []
    ...
    def removeRental(self, id = -1, bookID = -1, clientID = -1, bookList = []):
        if id > 0:
            ...
            for rental in self._list:
                if rental.getID() != id:
                    self._list1.append(rental)
            self._list = self._list1
        elif bookID > 0:
            self._list2 = []
            for rental in self._list:
                if rental.getBookID() != bookID:
                    self._list2.append(rental)
            self._list = self._list2
        elif clientID > 0:
            self._list3 = []
            for rental in self._list:
                if rental.getClientID() != clientID:
                    self._list3.append(rental)
                else:
                    for book in bookList:
                        if book.getID() == rental.getBookID() and rental.getReturnedDate() == "":
                            book.setAvailability(True)
            self._list = self._list3
